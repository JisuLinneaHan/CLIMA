...
@kernel function volumerhs!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder}, ::VerticalDirection,
                            rhs, Q, Qvisc, Qhypervisc_grad, auxstate, vgeo, t,
...
See [`odefun!`](@ref) for usage.
""" facerhs!
@kernel function facerhs!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder}, ::direction,
                          numfluxnondiff::NumericalFluxNonDiffusive,
                          numfluxdiff::NumericalFluxDiffusive,
...
@kernel function volumeviscterms!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder},
  ...
  @inbounds begin
    ...
    gradvariables!(bl, Vars{vars_gradient(bl,FT)}(l_G), Vars{vars_state(bl,FT)}(l_Q),
               Vars{vars_aux(bl,FT)}(l_aux), t)
...
@kernel function faceviscterms!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder},
...
@kernel function initstate!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder}, state, auxstate, vgeo, elems, args...) where {dim, polyorder}
  N = polyorder
  FT = eltype(auxstate)
  nauxstate = num_aux(bl,FT)
  nstate = num_state(bl,FT)
...
""" initauxstate!
@kernel function initauxstate!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder}, auxstate, vgeo, elems) where {dim, polyorder}
  N = polyorder
  FT = eltype(auxstate)
  nauxstate = num_aux(bl,FT)
...
@kernel function knl_indefinite_stack_integral!(bl::BalanceLaw, ::Val{dim}, ::Val{N},
                                                ::Val{nvertelem}, Q, auxstate, vgeo,
                                                Imat, elems) where {dim, N, nvertelem}
  @uniform begin
    FT = eltype(Q)
...
@kernel function knl_reverse_indefinite_stack_integral!(bl::BalanceLaw,
                                                        ::Val{dim}, ::Val{N},
                                                        ::Val{nvertelem},
                                                        state, auxstate,
                                                        elems
...
@kernel function knl_copy_stack_field_down!(::Val{dim}, ::Val{N}, ::Val{nvertelem},
...
@kernel function volumedivgrad!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder},
...
@kernel function facedivgrad!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder},
                              ::direction,
                              divgradnumpenalty,
...
@kernel function volumehyperviscterms!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder},
                                       ::direction,
...
@kernel function facehyperviscterms!(bl::BalanceLaw, ::Val{dim}, ::Val{polyorder},
                                     ::direction,
                                     hyperviscnumflux,
                                     Qhypervisc_grad, Qhypervisc_div,
                                     Q, auxstate,
...
@kernel function knl_local_courant!(bl::BalanceLaw, ::Val{dim}, ::Val{N},
